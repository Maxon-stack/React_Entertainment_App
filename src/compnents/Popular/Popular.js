...
const Popular = () => {
  ...
  const {popMovies, openPopUp, setPopUpItems, PopUpItems} = useContext(MovieContext);
  ...
  return(
      <div css = {styles} className = "popMovies">
        <Container>
          {popMovies.results && popMovies.results.map((popMovieItem, index) => (
            <img onClick ={() => { 
              const id = popMovies.results[index].id;
              //setSearchId(popMovies.results[index].id);
              openPopUp();
              console.log("Clicked Popular Movie Image ");
              //console.log(popMovieItem.poster_path);
              console.log(popMovies.results[index].id)
              
              console.log(id)

              handleSearchPopUp(popMovies.results[index].id);

              


            }} 
            key = {index} src = {`https://image.tmdb.org/t/p/w400/${popMovieItem.poster_path}`} alt = "Poster"/>
